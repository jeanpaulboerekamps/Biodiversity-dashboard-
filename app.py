...
with tab_dashboard:
    ...
    if st.button("🔎 Analyseer dit gebied", type="primary"):
        ...
        st.bar_chart(df["soortgroep"].value_counts())

        st.subheader("Ontwikkeling")
        yearly = df.groupby("jaar").agg(
        ...
        top = df.groupby(["soort", "wetenschappelijke naam"]).size().reset_index(name="waarnemingen")
        st.dataframe(top.sort_values("waarnemingen", ascending=False).head(30), use_container_width=True, hide_index=True)

        st.subheader("Alle waarnemingen")
        st.dataframe(
            df.sort_values("datum", ascending=False),
            use_container_width=True, hide_index=True,
            column_config={"url": st.column_config.LinkColumn("iNaturalist")}
        )
        st.download_button("⬇️ Download als CSV", df.to_csv(index=False).encode("utf-8"),
                           f"{active.replace(' ', '_')}_waarnemingen.csv", "text/csv")

        if total > 10000:
            st.warning("De zoekopdracht bevat meer dan 10.000 resultaten. Verklein gebied of periode voor volledige dekking.")

st.caption("iPad-prototype · Gebieden blijven tijdens de actieve sessie beschikbaar en kunnen als GeoJSON worden geëxporteerd.")
